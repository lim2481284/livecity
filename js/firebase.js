...
function Api(){
	...
	this.getEventForList=function(){
		...
		events.on("child_added",function(snap){
			...
			$('#list').append("<a style='cursor:pointer' class='event_row' key="+snap.key+"><div class='event_list_row' style=''><img id='icon' style='width:20vw;height:10vh;float:left;margin:5px;padding:5px;' src="+icon+"><h3>"+snap.val().name+"</h3><p style='width:40%'>"+snap.val().description+"</p></div></a><br>")
